...
The Service Reminder webhook notifies your system when a notification rule is triggered indicating that a service reminder is still due for a patient. Oliver sends an HTTP POST request to your configured endpoint with a JSON payload containing the reminder details.
...
<ResponseField name="practice_id" type="string" required>
  The unique identifier for the veterinary practice.
</ResponseField>
